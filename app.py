import os
# Vercel Server Crash Fix (Forces Python Protobuf instead of C++)
...
def get_access_token(account_str: str):
    ...
    with httpx.Client() as client:
        resp = client.post(url, content=payload.encode('utf-8'), headers=headers, timeout=5.0)
...
def create_jwt_for_account(idx: int, account_str: str):
    ...
    with httpx.Client() as client:
        resp = client.post(url, content=payload, headers=headers, timeout=5.0)
...
def GetAccountInformation(uid, unk, endpoint):
    ...
    with httpx.Client() as client:
        resp = client.post(server + endpoint, content=data_enc, headers=headers, timeout=5.0)
...
def get_account_info():
    ...
    try:
        ...
        return jsonify({"error": "API Execution Failed", "details": str(e)}), 500
