...
# Instance-level cache pool (survives as long as Vercel keeps instance alive)
token_pool = {}
...
async def create_jwt_for_account(idx: int, account_str: str):
    try:
        # Prevent exact simultaneous hits to login server
...
async def get_account_token_safely(idx: int) -> Tuple[str, str, str]:
    """Helper to fetch or retrieve token for specific index"""
...
async def GetAccountInformation(uid, unk, endpoint, max_retries=4):
    ...
    # Create a list of all bot account indices [0, 1, 2, 3, 4, 5, 6, 7]
    candidate_indices = list(range(len(ACCOUNTS)))
    ...
    for attempt in range(max_retries):
        ...
        # 1. Randomly pick an index from remaining pool
        idx = random.choice(candidate_indices)
        candidate_indices.remove(idx) # Don't try same index again in this request
        
        try:
            # 2. Try to get token (uses cache or logs in)
            token, lock, server = await get_account_token_safely(idx)
            ...
            async with httpx.AsyncClient(timeout=8.0) as client:
                ...
                # If token was manually revoked (401), force clean and try loop again
                if resp.status_code == 401:
                    if idx in token_pool:
                        del token_pool[idx]
                    continue 
            ...
            last_error = e
            print(f"Vercel Attempt {attempt+1} failed with account {idx}. Choosing another account... Error: {str(e)}")
            continue
            
    raise RuntimeError(f"All random attempts failed on Vercel. Last error: {str(last_error)}")
...
def home():
    return jsonify({
        ...
        "Version": "6.0.0 (Vercel Specialized Failover)",
        "Author": "Zero Gravity",
