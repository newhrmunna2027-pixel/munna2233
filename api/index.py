...
try:
    ...
    # Safely extract Protobuf Messages 
    _messages = message_factory.GetMessages([DESCRIPTOR_1, DESCRIPTOR_2, DESCRIPTOR_3])

    def get_msg_class(suffix):
        for name, cls in _messages.items():
            if name.endswith(suffix):
                return cls
        raise KeyError(f"Protobuf message '{suffix}' not found. Available: {list(_messages.keys())}")

    AccountPersonalShowInfo = get_msg_class('AccountPersonalShowInfo')
    LoginReq = get_msg_class('LoginReq')
    LoginRes = get_msg_class('LoginRes')
    GetPlayerPersonalShow = get_msg_class('GetPlayerPersonalShow')
    ...
    ACCOUNTS =[
        "uid=4744822452&password=OUT_OF_LAW_47145DFBD348F5F2D68B5D75AFA957AF10BE04EF89332B5B8E5EA",
        "uid=4744822456&password=OUT_OF_LAW_6C6E2DB1FE8C6087D999992B683D28E2ABF6B782E9B1CBDD2B9B0",
    ...
    def get_access_token(account_str: str):
        ...
        with httpx.Client() as client:
            resp = client.post(url, content=payload.encode('utf-8'), headers=headers, timeout=4.0)
    ...
    def create_jwt_for_account(idx: int, account_str: str):
        ...
        body = json.dumps({
            "open_id": open_id, "open_id_type": "4", "login_token": token_val, "orign_platform_type": "4"
        })
        ...
        with httpx.Client() as client:
            resp = client.post(url, content=payload, headers=headers, timeout=4.0)
            if resp.status_code != 200 or not resp.content or b'AUTH_FAILED' in resp.content:
                raise RuntimeError(f"Garena Auth Failed (Account {idx})")
    ...
    def get_rotated_token_info() -> Tuple[int, str, str, str]:
        ...
        if not info or time.time() >= info['expires_at']:
            info = create_jwt_for_account(idx, ACCOUNTS[idx])
    ...
    def GetAccountInformation(uid, unk, endpoint):
        ...
        headers = {
            'User-Agent': USERAGENT, 'Content-Type': "application/octet-stream",
            'Authorization': token, 'X-Unity-Version': "2018.4.11f1", 'X-GA': "v1 1", 'ReleaseVersion': RELEASEVERSION
        }
        
        with httpx.Client() as client:
            resp = client.post(server + endpoint, content=data_enc, headers=headers, timeout=4.0)
    ...
    def cached_endpoint(ttl=300):
        def decorator(fn):
            ...
            def wrapper(*a, **k):
                ...
                res = fn(*a, **k)
                if res[1] == 200: cache[key] = res
                return res
            return wrapper
        return decorator

except Exception as e:
    # IF ANYTHING FAILS DURING IMPORT/SETUP, SAVE THE ERROR
...
def home():
    if INIT_ERROR:
        return f"<h3 style='color:red;'>System Crash - Detailed Log:</h3><pre>{INIT_ERROR}</pre>", 500
    return jsonify({"App Name": "Info Flask Api", "Status": "Active"})

@app.route('/player-info')
# @cached_endpoint(ttl=300) # (Optional: remove comment to enable cache)
def get_account_info():
    if INIT_ERROR:
        return jsonify({"error": "Server crashed during setup. Visit the root '/' to see the error details.", "trace": INIT_ERROR}), 500
    ...
    try:
        data = GetAccountInformation(uid, "7", "/GetPlayerPersonalShow")
        return json.dumps(data, indent=2, ensure_ascii=False), 200, {'Content-Type': 'application/json'}
    except Exception as e:
        err_msg = traceback.format_exc()
        return jsonify({"error": "API Execution Failed", "details": str(e), "traceback": err_msg}), 500
