...
# --- Protobuf Definitions (Cleaned and Merged) ---
# Protobuf from AccountPersonalShow_pb2.py
...
# Build the Protobuf messages and enums into the global scope
_globals = globals()
_builder.BuildMessageAndEnumDescriptors(DESCRIPTOR_1, _globals)
_builder.BuildTopDescriptorsAndMessages(DESCRIPTOR_1, 'AccountPersonalShow_pb2', _globals)
_builder.BuildMessageAndEnumDescriptors(DESCRIPTOR_2, _globals)
_builder.BuildTopDescriptorsAndMessages(DESCRIPTOR_2, 'FreeFire_pb2', _globals)
_builder.BuildMessageAndEnumDescriptors(DESCRIPTOR_3, _globals)
_builder.BuildTopDescriptorsAndMessages(DESCRIPTOR_3, 'main_pb2', _globals)
...
async def get_access_token(account_str: str):
    ...
    # Increased timeout for serverless environment
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if resp.status_code != 200 or not resp.content or resp.content.startswith(b'BR_GOP_TOKEN_AUTH_FAILED'):
                raise RuntimeError(f"Token request failed for account index {idx} with status {resp.status_code}")
...
async def GetAccountInformation(uid, unk, endpoint):
    # Ensure UID and unk are converted to integers for the protobuf message
    payload = await json_to_proto(json.dumps({'a': int(uid), 'b': int(unk)}), GetPlayerPersonalShow())
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        # Raise an exception for other bad responses to see the error in logs
        resp.raise_for_status()
...
def home():
    return jsonify({
        "AppName": "Info Flask Api", "Version": "4.2.0-Vercel-Fix", "Author": "Zero Gravity",
        "EndPoint": "/player-info?uid=1765197992", "AccountsPool": f"{len(ACCOUNTS)} Accounts Loaded",
        "Developer": "Shuvo", "Copyright": "© 2025 Zero Gravity - All rights reserved.",
        "Region": "Global", "Discord Server": "Zero Gravity",
...
def get_account_info():
    ...
    try:
        # A better way to run async code inside Flask for Vercel
...
if __name__ == '__main__':
    print(f"🚀 Starting App with {len(ACCOUNTS)} accounts in Round-Robin mode for LOCAL testing.")
