...
def install_missing_packages():
    required_packages = {
        ...
        "google.protobuf": "protobuf==6.30.0"
    }
...
# আপনার দেওয়া ১০টি অ্যাকাউন্টের লিস্ট 
ACCOUNTS = [
...
async def create_jwt_for_account(idx: int, account_str: str):
    """Generates and saves the JWT token for a specific account index."""
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            token_pool[idx] = {
                ...
                'expires_at': time.time() + 25200  # 7 hours expiration
            }
...
async def get_rotated_token_info() -> Tuple[int, str, str, str]:
    """Gets the next account, creates token if missing/expired, and returns it."""
...
if __name__ == '__main__':
    print(f"🚀 Starting App with {len(ACCOUNTS)} accounts in Round-Robin mode.")
